...
local restoreVGroupTabSettings = function()
  ...
  local maxWindowId = 0;
  if (type(settings.windowStates) == "table") then
    for id,_ in pairs(settings.windowStates) do
      local n = tonumber(id) or 0;
      if (n > maxWindowId) then maxWindowId = n end
    end
  end
  if (type(settings.statsWindowStates) == "table") then
    for id,_ in pairs(settings.statsWindowStates) do
      local n = tonumber(id) or 0;
      if (n > maxWindowId) then maxWindowId = n end
    end
  end
  local groupWindowId = maxWindowId + 1;

  -- create a dedicated window for the group tab
  if (settings.windowStates == nil) then settings.windowStates = {} end
  settings.windowStates[groupWindowId] = {
    ["x"] = nil, ["y"] = nil, ["w"] = 270, ["h"] = 221,
    ["showBackground"] = true, ["showTitleBar"] = true,
    ["autoHideTabs"] = false, ["minimized"] = false,
    ["backgroundColor"] = Turbine.UI.Color(0.046154,0.4,0.4,0.4)
  };

  -- register the group tab with matching defaults so interpretSettings renders it
  if (settings.tabStates == nil) then settings.tabStates = {} end
  settings.tabStates["groupTab"] = {
    ["indexInWindow"] = 1, ["selected"] = true, ["windowID"] = groupWindowId,
    ["statsWindowID"] = nil, ["statsWindowVisibility"] = "Hide", ["autoSelectPlayer"] = false,
    ["showTitle"] = true, ["showEncounters"] = true, ["showTargets"] = true, ["showExtraButtons"] = true,
    ["display"] = "Totals",
    ["colorScheme"] = Turbine.UI.Color(0.4,0.6,0.3,0.9),
    ["expandedStatHeadings"] = {
      ["totals"] = true, ["crits"] = true,
      ["avoidance"] = true, ["fullAvoids"] = true, ["partials"] = true,
      ["other"] = true, ["dmgTypes"] = true
    }
  };
end
